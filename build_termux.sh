...
export PATH="/data/user/0/com.termux/files/usr/lib/ccache:$PATH"
echo "CCACHE_DIR: [$CCACHE_DIR]"


MAKE_ARGS="ARCH=arm64 SUBARCH=arm64 O=out LLVM=1"
CFLAGS="--target=aarch64-linux-android --sysroot=/data/user/0/com.termux/files -O3 -march=armv8.2-a+lse+crypto+dotprod -mcpu=cortex-a77 -flto -Wno-error"
...
scripts/config --file out/.config \
    ...
    -e CONFIG_ZRAM_BACKEND_LZ4 \
    -d CONFIG_ZRAM_BACKEND_LZ4HC \
    -d CONFIG_ZRAM_BACKEND_ZSTD \
    -d CONFIG_ZRAM_BACKEND_LZO \
    -e CONFIG_ZRAM_DEF_COMP_LZ4

make CFLAGS="$CFLAGS" CXXFLAGS="$CFLAGS" $MAKE_ARGS -j16
